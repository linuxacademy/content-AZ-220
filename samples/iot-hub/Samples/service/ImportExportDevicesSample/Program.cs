...
namespace Microsoft.Azure.Devices.Samples
{
    public class Program
    {
        ...
        // They are the first six command-line arguments, and all are required 
        ...
        // Export devices from the source hub ONLY!
        private static bool _exportOnly = true;
        ...
        private static string _envexportOnly = Environment.GetEnvironmentVariable("EXPORT_ONLY");
        ...
        public static async Task Main(string[] args)
        {
            ...
            // **exportOnly**
            if (string.IsNullOrEmpty(_envexportOnly) && args.Length > 1)
            {
                _envexportOnly = args[1];
            }
            if (!string.IsNullOrEmpty(_envexportOnly))
            {
                _exportOnly = _envexportOnly.Trim().ToUpper() == "TRUE" ? true : false;
            }

            // **numToAdd**
            if (_addDevices)
            {
                if (string.IsNullOrEmpty(_envNumToAdd) && args.Length > 2)
                {
                    _envNumToAdd = args[2];
                }
            ...
            // **copyDevices**
            if (string.IsNullOrEmpty(_envCopyDevices) && args.Length > 3)
            {
                _envCopyDevices = args[3];
            }
            ...
            // **deleteSourceDevices**
            if (string.IsNullOrEmpty(_envDeleteSourceDevices) && args.Length > 4)
            {
                _envDeleteSourceDevices = args[4].Trim().ToUpper();
            }
            ...
            // **deleteDestDevices**
            if (string.IsNullOrEmpty(_envDeleteDestDevices) && args.Length > 5)
            {
                _envDeleteDestDevices = args[5];
            }
            ...
            if (string.IsNullOrEmpty(_envIotHubConnectionString) && args.Length > 6)
            {
                _envIotHubConnectionString = args[6];
            }

            // ** DestIoTHubConnectionString **
            if (string.IsNullOrEmpty(_envIotHubConnectionString) && args.Length > 6)
            {
                _envDestIotHubConnectionString = args[6];
            }

            // ** storageAccountConnectionString **
            if (string.IsNullOrEmpty(_envStorageAccountConnectionString) && args.Length > 6)
            {
                _envStorageAccountConnectionString = args[6];
            }

            // Show the data passed in and what it thinks it was.
            Console.WriteLine("Input:");
            Console.WriteLine($"  add devices = {_addDevices}.");
            Console.WriteLine($"  export only = {_exportOnly}.");
            ...
            try
            {
                await importExportDevicesSample
                    .RunSampleAsync(_addDevices, _exportOnly, _numToAdd, _copyDevices, _deleteSourceDevices, _deleteDestDevices)
